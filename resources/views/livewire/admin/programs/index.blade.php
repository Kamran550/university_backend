<div class="p-6 max-w-7xl mx-auto" x-data="{ showModal: false, showEditModal: false, showDeleteModal: false }" @close-modal.window="showModal = false"
    ...
    @program-created.window="showModal = false; $wire.$refresh()"
    @program-updated.window="showEditModal = false; $wire.$refresh()"
    @program-deleted.window="showDeleteModal = false; $wire.$refresh()">
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    @php
                                        $availableLanguages = $program->studyLanguages
                                            ->filter(fn($lang) => $lang->is_available)
                                            ->pluck('language')
                                    ...
                                    @if (count($availableLanguages) > 0)
                                        @foreach ($availableLanguages as $lang)
                                            <span
                                                ...
                                                @if (strtolower($lang) === 'en') bg-blue-100 text-blue-800 @else bg-green-100 text-green-800 @endif">
                                                {{ $lang }}
                                            </span>
                                        @endforeach
                                    @else
